...
from models_cnnWithHints import Model
...
def configure_dataloaders(train_batch_size=1, eval_batch_size=16, shuffle=False):
    "Prepare dataloaders"
...
def expand_batch(batch, input_format):
    ...
    for i, (img, q, choices, h, a) in enumerate(zip(*batch)):
        ...
        hints += [h[:100] for j in range(len(choices))]
        y = [0]*len(choices)
        y[a] = 1
        labels += y
#         print("len of hints : ", len(hints))
#         print("len of content : ", len(content))
    
    return images, content, hints, labels
...
def train_or_eval_model(model, dataloader, optimizer=None, input_format="0", split="Train"):
    ...
    for batch in tqdm(dataloader, leave=False):
        ...
        images, content, hints, l_cls = expand_batch(batch, input_format)
        #print(len(images), len(content), len(l_cls))
        loss, p, p_cls = model((images, content, hints, l_cls))
