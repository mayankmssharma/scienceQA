...
class ScienceQADataset(Dataset):
    def __init__(self, data, shuffle):
        data = pd.DataFrame(data)[["question","choices","answer","hint","image"]]
        if shuffle:
        ...
        self.images = list(data["image"].values)
    ...
    def __getitem__(self, index):
        s0, s1, s2, s3, s4 = self.images[index], self.question[index], self.choices[index], self.hint[index], self.answer[index]
...
def configure_dataloaders(train_batch_size=16, eval_batch_size=16, shuffle=False):
    ...
    dataTrain = pd.read_pickle("./resnet152ImageEmbeddings/train_All152.pkl")
    dataVal = pd.read_pickle("./resnet152ImageEmbeddings/validation_All152.pkl")
    dataTest = pd.read_pickle("./resnet152ImageEmbeddings/test_All152.pkl")

    train_dataset = ScienceQADataset(dataTrain, True)
    train_loader = DataLoader(train_dataset, shuffle=shuffle, batch_size=train_batch_size, collate_fn=train_dataset.collate_fn)

    val_dataset = ScienceQADataset(dataVal, False)
    val_loader = DataLoader(val_dataset, shuffle=False, batch_size=eval_batch_size, collate_fn=val_dataset.collate_fn)
    
    test_dataset = ScienceQADataset(dataTest, False)
...
def expand_batch(batch, input_format):
    ...
    images, content, labels = [], [], []
    img, qs, cs, hs, ans = batch
    counter = 0
    #print(batch, len(batch))
    for i, (img, q, choices, h, a) in enumerate(zip(*batch)):
        #print(img, q, choices, h, a)

        counter += 1
        # if(counter > noOfSamples):
        #     break
        ...
        #     img = torch.tensor(np.asarray(input_transform(img)))
        #     images += [img for j in range(len(choices))]
        #     #print("stacking - ", len(images))
        #     #images = torch.stack(images)
        # else:
            
        #     images+= [None for j in range(len(choices))]
        images += [torch.from_numpy(img) for j in range(len(choices))]
        y = [0]*len(choices)
        y[a] = 1
        labels += y
    
    return images, content, labels
...
def train_or_eval_model(model, dataloader, optimizer=None, input_format="0", split="Train"):
    ...
    for batch in tqdm(dataloader, leave=False):
        ...
        #print(batch[1])
        sizes = [len(x) for x in batch[2]]
        
        images, content, l_cls = expand_batch(batch, input_format)
        #print(len(images), len(content), len(l_cls))
        loss, p, p_cls = model((images, content, l_cls))
