...
class MultiScienceQADataset(Dataset):
    def __init__(self, data, shuffle):
        data = pd.DataFrame(data)[["question","choices","answer","hint","img_embed"]]
        if shuffle:
        ...
        self.img_embed = list(data["img_embed"].values)
    ...
    def __getitem__(self, index):
        s1, s2, s3, s4, s5 = self.question[index], self.choices[index], self.hint[index], self.answer[index], self.img_embed[index]
...
def expand_batch(batch, input_format):
    content, img_embeds, labels = [], [], []
    for i, (q, choices, h, a, ie) in enumerate(zip(*batch)):
        #Plain text
        if input_format == "0":
            for c in choices:
        ...
        #Append img_embed for every choice        
        for c in choices:
    ...
    return content, img_embeds, labels
...
def train_or_eval_model(model, dataloader, optimizer=None, input_format="0", split="Train"):
    ...
    for batch in tqdm(dataloader, leave=False):
        ...
        content, img_embeds, l_cls = expand_batch(batch, input_format)
        loss, p, p_cls = model((content, img_embeds, l_cls))
